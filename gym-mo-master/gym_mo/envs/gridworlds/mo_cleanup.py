...
class MOCleanupCustomEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    MAX_STEPS = 500

    def __init__(self, n_agent=2, image=True):
        super(MOCleanupCustomEnv, self).__init__()
        ...
        if self.image:
            ...
            self.observation_shape = (2 + 3*self.n_agent ,)
    ...
    def _reset_agents(self):
        self.agents = []
        for i_agent, agent_type in enumerate(["prey"] + ["predator" for _ in range(self.n_agent )]):
    ...
    def step(self, actions):
        assert len(actions) == self.n_agent  + 1
...
if __name__ == '__main__':
    N_AGENT  = 2
    env = MOWolfpackCustomEnv(n_agent =N_AGENT , image=False)
    ...
    while True:
        ...
        pred_actions = [env.action_space.sample() for _ in range(N_AGENT )]
